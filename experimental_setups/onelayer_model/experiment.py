...
for model_idx in range(1):
    ...
    save_file = f'onelayer-bhvsim.torch'

    # model training
    errors = calc_model_error(simulation_model,train_x,train_y)
    c=1
    while errors.mean() > 0.18 and c<1001:
        if c%100==0:
            print(f'Running epoch {c}...\terrors={errors.mean()}\n')
...
error_data.to_csv(f'data/onelayer_simulation_data_0200.csv')
